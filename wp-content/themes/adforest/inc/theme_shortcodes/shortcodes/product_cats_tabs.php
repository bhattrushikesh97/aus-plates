...
/* Shop Modern 5 */
/* ------------------------------------------------ */
if (!function_exists('products_cats_tabs_data_shortcode2')) {

    function products_cats_tabs_data_shortcode2($term_type = 'ad_country') {
        $result = array();
        if (!is_admin()) {
            return $result;
        }
...
if (!function_exists('products_cats_tabs5_short')) {

    function products_cats_tabs5_short() {
        vc_map(array(
            "name" => __("Product cats tabs", 'adforest'),
            "base" => "products_cats_tabs5_short_base",
            ...
            "params" => array(
                ...
                array(
                    ...
                    "holder" => "bg_img",
                    "class" => "",
                    "heading" => __("Banner image 285x395", 'adforest'),
                ...
                array(
                    ...
                    "param_name" => "banner_postion",
                    "value" => array(
                        __('Left', 'adforest') => 'left',
                        __('Right', 'adforest') => 'right',
                    ),
                ),
                array(
...
add_action('vc_before_init', 'products_cats_tabs5_short');
if (!function_exists('products_cats_tabs5_short_base_func')) {

    function products_cats_tabs5_short_base_func($atts, $content = '') {
        ...
        if (isset($adforest_elementor) && $adforest_elementor) {
            $banner_image = isset($atts ['banner_img']['url']) ? $atts ['banner_img']['url'] : '';
        } else {
            $banner_image = isset($atts ['banner_img']) ? adforest_returnImgSrc($atts ['banner_img']) : '';
        }

        $left_banner = $right_banner = "";
        

        if ($banner_postion != "right") {

            $left_banner = '<div class="col-lg-3 col-md-6 col-sm-12 col-xs-12">
                                    <div class="fashion-sale">
                                    <img src="' . $banner_image . '" alt="sale img">
                                     </div>
                                </div>';
        } else {

            $right_banner = '<div class="col-lg-3 col-md-6 col-sm-12 col-xs-12">
                                    <div class="fashion-sale">
                                    <img src="' . $banner_image . '" alt="sale img">
                                     </div>
                                </div>';
        ...
        if (is_array($woo_products) && !empty($woo_products)) {
            ...
            foreach ($woo_products as $woo_product) {
                ...
                if ($categ_id != '' && $categ_count > 0) {
                    ...
                    $cat_posts_html .= ' <div class="tab-pane fade show ' . $is_active_tab . '" id="' . $rand . '" role="tabpanel" aria-labelledby="home-tab"><div class="owl-carousel1 products_tabs_slider staff-list">';
                    $cat_posts_html .= get_products_by_category($categ_id, $max_limit, 'term_id');
                    $cat_posts_html .= '</div></div>';
                }
            }
        }
        return '<section class="latest-product">
    <div class="container">
        <div class="row">
             ' . $left_banner . '
...
if (function_exists('adforest_add_code')) {
    adforest_add_code('products_cats_tabs5_short_base', 'products_cats_tabs5_short_base_func');
}